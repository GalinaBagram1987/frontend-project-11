...
const config = {
  ...
  module: {
    rules: [
      ...
      { test: /.woff2?(\?v=[0-9].[0-9].[0-9])?$/, type: 'asset/resource' },
      { test: /.(ttf|eot|svg)(\?[\s\S]+)?$/, type: 'asset/resource' },
    ],
  },
  plugins: [new HtmlWebpackPlugin({ template: 'index.html' })],
  output: { clean: true, filename: 'main.js', path: path.resolve(__dirname, 'dist') },
  optimization: {
    minimize: false, // Отключаем в development
  },
  watchOptions: {
